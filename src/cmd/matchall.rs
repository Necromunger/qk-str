...
pub const USAGE: &str = "qk matchall <string> <pattern> — all regex matches, one per line";
...
mod tests {
    ...
    #[test]
    fn multiple() {
        assert_eq!(all_matches("a1b2c3", r"\d"), vec!["1", "2", "3"]);
    }

    #[test]
    fn none() {
        assert!(all_matches("abc", r"\d").is_empty());
    }

    #[test]
    fn single() {
        assert_eq!(all_matches("hello123", r"\d+"), vec!["123"]);
    }
}
